...
for(elem in types_list) {
  ...
  heat_max = max(na.omit(heatmap_matrix)) #+ eps
  ...
  res <- plot(
    ...
    #breaks = c(seq(min(heat_min, 0-k), max(heat_min, 0-k), -(heat_min+k)/N),
    #           seq(min(0-k, heat_max), max(0-k, heat_max), abs((heat_max+k) / (N-1)))),
    ...
    axis.key=NULL, 
    border= NA,
    spacing.key=c(1,0.5,0) #spacing.key(venstre, tykkelse, h?jre)
  )
